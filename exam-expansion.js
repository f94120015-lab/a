...
(function () {
  ...
  const T = [
    { key: 'scribes', tt: 'ortaçağ müstensihlerinin bıraktığı hatalar',
      i: ['Medieval workshops copied manuscripts entirely by hand.', 'Ortaçağ atölyeleri el yazmalarını tamamen elle kopyalıyordu.', ['Ortaçağ atölyeleri el yazmalarını basım makineleriyle çoğaltıyordu.', 'Ortaçağ müstensihleri el yazmalarını yalnızca kısmen kopyalıyordu.', 'Modern atölyeler el yazmalarını tamamen elle kopyalıyor.', 'Ortaçağ atölyeleri el yazmalarını elle düzenliyordu, kopyalamıyordu.']],
      d: ['Every copyist left small errors that belonged to his own hand alone.', 'Her müstensih yalnızca kendi eline özgü küçük hatalar bırakıyordu.', ['Bazı müstensihler kendi eline özgü küçük hatalar bırakıyordu.', 'Her müstensih başka atölyelerle ortak hatalar bırakıyordu.', 'Her müstensih büyük hatalar bırakıyordu, ama bunlar fark edilmiyordu.', 'Her müstensih zamanla hatalarını düzeltiyordu.']],
      e: ['By grouping these errors, researchers can tell which workshop produced a given copy.', 'Bu hataları gruplayan araştırmacılar bir kopyanın hangi atölyeden çıktığını belirleyebiliyor.', ['Bu hataları gruplayan araştırmacılar kopyanın hangi yüzyıldan kaldığını belirleyebiliyor.', 'Araştırmacılar bu hataları gruplamadan da hangi atölyeden çıktığını belirleyebiliyor.', 'Bu hataları gruplayan müstensihler kopyanın atölyesini gizleyebiliyor.', 'Bu hatalar gruplandığında kopyanın değerini artırıyor.']],
      c: ['What earlier scholars dismissed as damage therefore serves today as evidence.', 'Bu yüzden önceki araştırmacıların hasar sayıp attığı şey bugün kanıt işlevi görüyor.', ['Bu yüzden önceki araştırmacıların kanıt saydığı şey bugün hasar olarak görülüyor.', 'Buna rağmen önceki araştırmacıların hasar sayıp attığı şey bugün de hasar sayılıyor.', 'Önceki araştırmacılar bu hasarı henüz incelemedi.', 'Bu yüzden gelecekteki araştırmacılar bunu kanıt olarak kullanacak.']],
      x: ['The same errors, however, make a single manuscript useless as a source of the original wording.', 'Ne var ki aynı hatalar, tek bir el yazmasını özgün metnin kaynağı olarak kullanılamaz kılıyor.', ['Bu sayede aynı hatalar, tek bir el yazmasını özgün metnin en güvenilir kaynağı yapıyor.', 'Ne var ki farklı hatalar, birden fazla el yazmasını özgün metnin kaynağı olarak kullanılamaz kılıyor.', 'Ne var ki aynı hatalar, el yazmasının yaşını belirlemeyi kolaylaştırıyor.', 'Ne var ki aynı hatalar zamanla kendiliğinden düzeliyor.']],
    ...
    { key: 'icecores', tt: 'buz karotlarının tuttuğu iklim kaydı',
      i: ['Ice cores drilled from polar glaciers preserve a layer for every year of snowfall.', 'Kutup buzullarından çıkarılan buz karotları her yılın karına ait bir katman saklar.', ['Kutup buzullarından çıkarılan buz karotları yalnızca son yüz yılın katmanlarını saklar.', 'Ekvator buzullarından çıkarılan buz karotları her yılın karına ait bir katman saklar.', 'Kutup buzullarından çıkarılan buz karotları her yılın yağmuruna ait bir katman saklar.', 'Kutup buzulları eridiğinde her yılın kar katmanı ortaya çıkar.']],
      d: ['Air trapped in those layers keeps a sample of the atmosphere of that year.', 'Bu katmanlarda hapsolan hava o yılın atmosferinden bir örnek tutar.', ['Bu katmanlarda hapsolan su o yılın atmosferinden bir örnek tutar.', 'Bu katmanlarda hapsolan hava yalnızca yakın zamanlı atmosferi yansıtır.', 'Bu katmanlardan sızan hava atmosfer örneğini kaybeder.', 'Bu katmanlarda hapsolan hava gelecekteki iklimi tahmin etmede kullanılır.']],
      e: ['Measurements of the trapped gas show how carbon dioxide rose after 1850.', 'Hapsolmuş gazın ölçümleri karbondioksitin 1850 sonrasında nasıl yükseldiğini gösteriyor.', ['Hapsolmuş gazın ölçümleri karbondioksitin 1850 öncesinde nasıl yükseldiğini gösteriyor.', 'Hapsolmuş gazın ölçümleri oksijenin 1850 sonrasında nasıl yükseldiğini gösteriyor.', 'Hapsolmuş gazın ölçümleri karbondioksitin 1850 sonrasında nasıl düştüğünü gösteriyor.', 'Hapsolmuş gazın ölçümleri karbondioksit seviyesinin 1950\'den beri sabit kaldığını gösteriyor.']],
      c: ['The cores thus provide a climate record that no written source can match.', 'Böylece karotlar hiçbir yazılı kaynağın erişemeyeceği bir iklim kaydı sunuyor.', ['Böylece karotlar yazılı kaynaklarla aynı doğrulukta bir iklim kaydı sunuyor.', 'Böylece yazılı kaynaklar karotların erişemeyeceği bir iklim kaydı sunuyor.', 'Böylece karotlar iklim kaydı konusunda yazılı kaynaklardan daha az güvenilir.', 'Böylece karotlar yalnızca bölgesel sıcaklık kaydı sunuyor, iklim kaydı değil.']],
      x: ['Cores from warmer regions, by contrast, melt and lose their annual layers.', 'Buna karşılık ılıman bölgelerden alınan karotlar erir ve yıllık katmanlarını yitirir.', ['Buna karşılık soğuk bölgelerden alınan karotlar erir ve yıllık katmanlarını yitirir.', 'Bunun sonucunda ılıman bölgelerden alınan karotlar donar ve katmanlarını korur.', 'Buna karşılık ılıman bölgelerden alınan karotlar erir ama katmanlarını korur.', 'Buna karşılık ılıman bölgelerdeki buzullar hiç oluşmaz.']],
    ...
    { key: 'tramline', tt: 'yeniden açılan tramvay hattının kullanımı',
      i: ['The city reopened its nineteenth-century tram line in 2016.', 'Kent, on dokuzuncu yüzyıldan kalma tramvay hattını 2016\'da yeniden açtı.', ['Kent, yirminci yüzyıldan kalma tramvay hattını 2016\'da yeniden açtı.', 'Kent, on dokuzuncu yüzyıldan kalma tramvay hattını 2016\'da kapattı.', 'Kent, on dokuzuncu yüzyıldan kalma otobüs hattını 2016\'da yeniden açtı.', 'Kent, on dokuzuncu yüzyıldan kalma tramvay hattını 2006\'da yeniden açtı.']],
      d: ['Planners expected the line to serve mainly tourists in the old quarter.', 'Plancılar hattın esas olarak eski kentteki turistlere hizmet etmesini bekliyordu.', ['Plancılar hattın esas olarak eski kentteki sakinlere hizmet etmesini bekliyordu.', 'Plancılar hattın turistlere değil işe gidip gelenlere hizmet edeceğini biliyordu.', 'Plancılar hattın yeni kentteki turistlere hizmet etmesini bekliyordu.', 'Turistler hattın eski kentte açılmasını talep etmişti.']],
      e: ['Ticket data from the first three years showed that most passengers were commuters.', 'İlk üç yılın bilet verileri yolcuların çoğunun işe gidip gelenler olduğunu gösterdi.', ['İlk üç yılın bilet verileri yolcuların çoğunun turist olduğunu gösterdi.', 'İlk on yılın bilet verileri yolcuların çoğunun işe gidip gelenler olduğunu gösterdi.', 'İlk üç yılın bilet verileri yolcu sayısının beklenenden az olduğunu gösterdi.', 'İlk üç yılın bilet verileri yolcuların yarısının işe gidip gelenler olduğunu gösterdi.']],
      c: ['The council consequently extended the line towards the residential districts.', 'Bu nedenle belediye meclisi hattı konut bölgelerine doğru uzattı.', ['Bu nedenle belediye meclisi hattı turistik bölgelere doğru uzattı.', 'Buna rağmen belediye meclisi hattı uzatmadı.', 'Bu nedenle belediye meclisi hattın bir bölümünü kapattı.', 'Bu nedenle özel bir şirket hattı konut bölgelerine doğru uzattı.']],
      x: ['Bus routes along the same corridor, however, lost a quarter of their passengers.', 'Ne var ki aynı koridordaki otobüs hatları yolcularının dörtte birini kaybetti.', ['Ne var ki aynı koridordaki otobüs hatları yolcularının yarısını kaybetti.', 'Bunun sonucunda aynı koridordaki otobüs hatları yolcularının dörtte birini kazandı.', 'Ne var ki farklı koridorlardaki otobüs hatları yolcularının dörtte birini kaybetti.', 'Ne var ki aynı koridordaki tramvay hatları yolcularının dörtte birini kaybetti.']],
    ...
    { key: 'antibiotic', tt: 'yeni antibiyotik kaynakları arayışı',
      i: ['Soil bacteria produce compounds that kill competing micro-organisms.', 'Toprak bakterileri rakip mikroorganizmaları öldüren bileşikler üretir.', ['Toprak bakterileri rakip mikroorganizmaları besleyen bileşikler üretir.', 'Deniz bakterileri rakip mikroorganizmaları öldüren bileşikler üretir.', 'Toprak mantarları rakip mikroorganizmaları öldüren bileşikler üretir.', 'Toprak bakterileri yalnızca kendi türünü öldüren bileşikler üretir.']],
      d: ['Most antibiotics in clinical use were isolated from such soil samples.', 'Klinikte kullanılan antibiyotiklerin çoğu bu tür toprak örneklerinden elde edilmiştir.', ['Klinikte kullanılan antibiyotiklerin azı bu tür toprak örneklerinden elde edilmiştir.', 'Klinikte kullanılan antibiyotiklerin çoğu deniz örneklerinden elde edilmiştir.', 'Klinikte kullanılan antibiyotiklerin tamamı laboratuvarda sentezlenmiştir.', 'Toprak örneklerinin çoğu klinik antibiyotiklerden elde edilmiştir.']],
      e: ['Screening of the same soils stopped yielding new compounds after the 1960s.', 'Aynı toprakların taranması 1960\'lardan sonra yeni bileşik vermez oldu.', ['Aynı toprakların taranması 1960\'lardan önce yeni bileşik vermez oldu.', 'Farklı toprakların taranması 1960\'lardan sonra yeni bileşik vermez oldu.', 'Aynı toprakların taranması 1960\'lardan sonra daha çok yeni bileşik verdi.', 'Aynı toprakların taranması 1990\'lardan sonra yeni bileşik vermez oldu.']],
      c: ['Researchers therefore turned to marine sediments and deep caves for fresh candidates.', 'Bu yüzden araştırmacılar yeni adaylar için deniz tortullarına ve derin mağaralara yöneldi.', ['Bu yüzden araştırmacılar yeni adaylar için toprak örneklerine yeniden yöneldi.', 'Buna rağmen araştırmacılar deniz tortullarına ve derin mağaralara yönelmedi.', 'Bu yüzden araştırmacılar yeni adaylar için yalnızca derin mağaralara yöneldi.', 'Bu yüzden hastaneler yeni adaylar için deniz tortullarına yöneldi.']],
      x: ['Cultivating these organisms in a laboratory, however, remains extremely difficult.', 'Ne var ki bu organizmaları laboratuvarda üretmek son derece zor olmayı sürdürüyor.', ['Bu sayede bu organizmaları laboratuvarda üretmek oldukça kolaylaştı.', 'Ne var ki bu organizmaları doğal ortamında üretmek son derece zor olmayı sürdürüyor.', 'Ne var ki bu organizmaları laboratuvarda tanımlamak son derece zor olmayı sürdürüyor.', 'Ne var ki bu organizmaların laboratuvarda üretilmesine artık gerek kalmadı.']],
    ...
    { key: 'archive', tt: 'arşivin dijitalleştirilip dizinlenmesi',
      i: ['The national archive began digitising its Ottoman land registers in 2011.', 'Ulusal arşiv Osmanlı tapu defterlerini 2011\'de dijitalleştirmeye başladı.', ['Ulusal arşiv Osmanlı tapu defterlerini 2011\'de dijitalleştirmeyi tamamladı.', 'Ulusal arşiv Cumhuriyet dönemi tapu defterlerini 2011\'de dijitalleştirmeye başladı.', 'Ulusal arşiv Osmanlı tapu defterlerini 2001\'de dijitalleştirmeye başladı.', 'Yerel bir arşiv Osmanlı tapu defterlerini 2011\'de dijitalleştirmeye başladı.']],
      d: ['Each page was photographed and indexed by place name rather than by date.', 'Her sayfa fotoğraflandı ve tarihe göre değil yer adına göre dizinlendi.', ['Her sayfa fotoğraflandı ve yer adına göre değil tarihe göre dizinlendi.', 'Her sayfa taranarak dijitalleştirildi ama dizinlenmedi.', 'Sadece bazı sayfalar fotoğraflandı ve yer adına göre dizinlendi.', 'Her sayfa fotoğraflandı ve hem tarihe hem yer adına göre dizinlendi.']],
      e: ['Use of the collection rose threefold in the two years after the index appeared.', 'Dizin yayımlandıktan sonraki iki yılda koleksiyonun kullanımı üç katına çıktı.', ['Dizin yayımlanmadan önceki iki yılda koleksiyonun kullanımı üç katına çıktı.', 'Dizin yayımlandıktan sonraki iki yılda koleksiyonun kullanımı üçte bir azaldı.', 'Dizin yayımlandıktan sonraki beş yılda koleksiyonun kullanımı üç katına çıktı.', 'Dizin yayımlandıktan sonraki iki yılda koleksiyonun kullanımı iki katına çıktı.']],
      c: ['The archive has since applied the same method to its court records.', 'Arşiv o tarihten beri aynı yöntemi mahkeme kayıtlarına da uyguluyor.', ['Arşiv o tarihten beri farklı bir yöntemi mahkeme kayıtlarına uyguluyor.', 'Arşiv o tarihten beri aynı yöntemi mahkeme kayıtlarına uygulamayı bıraktı.', 'Mahkemeler o tarihten beri aynı yöntemi arşiv kayıtlarına uyguluyor.', 'Arşiv o tarihten beri aynı yöntemi yalnızca tapu defterlerine uyguluyor.']],
      x: ['Readers looking for a particular year, on the other hand, now search far more slowly.', 'Öte yandan belirli bir yılı arayan okurlar artık çok daha yavaş arama yapıyor.', ['Öte yandan belirli bir yılı arayan okurlar artık çok daha hızlı arama yapıyor.', 'Öte yandan belirli bir yeri arayan okurlar artık çok daha yavaş arama yapıyor.', 'Bu sayede belirli bir yılı arayan okurlar artık çok daha yavaş arama yapıyor.', 'Öte yandan belirli bir yılı arayan araştırmacılar artık arama yapamıyor.']],
    ...
    { key: 'coral', tt: 'mercan resiflerinde ağarma',
      i: ['Coral reefs lose their colour when the algae living in them are expelled.', 'Mercan resifleri içlerinde yaşayan algleri dışarı attıklarında rengini yitirir.', ['Mercan resifleri içlerinde yaşayan algleri içeri çektiklerinde rengini yitirir.', 'Mercan resifleri içlerinde yaşayan algleri dışarı attıklarında rengini kazanır.', 'Balıklar içlerinde yaşayan algleri dışarı attıklarında rengini yitirir.', 'Mercan resifleri su sıcaklığı düştüğünde rengini yitirir.']],
      d: ['The expulsion is triggered by a rise of only one or two degrees in sea temperature.', 'Bu atım, deniz sıcaklığındaki yalnızca bir iki derecelik yükselmeyle tetiklenir.', ['Bu atım, deniz sıcaklığındaki yalnızca bir iki derecelik düşüşle tetiklenir.', 'Bu atım, deniz sıcaklığındaki on derecelik bir yükselmeyle tetiklenir.', 'Bu atım, hava sıcaklığındaki bir iki derecelik yükselmeyle tetiklenir.', 'Bu atım, deniz tuzluluğundaki değişimle tetiklenir.']],
      e: ['Surveys after the 2016 heatwave recorded bleaching along two thirds of the reef.', '2016 sıcak hava dalgasından sonraki incelemeler resifin üçte ikisinde ağarma kaydetti.', ['2016 sıcak hava dalgasından sonraki incelemeler resifin üçte birinde ağarma kaydetti.', '2016 sıcak hava dalgasından önceki incelemeler resifin üçte ikisinde ağarma kaydetti.', '2019 sıcak hava dalgasından sonraki incelemeler resifin üçte ikisinde ağarma kaydetti.', '2016 sıcak hava dalgasından sonraki incelemeler resifin tamamının iyileştiğini kaydetti.']],
      c: ['Recovery is possible, but only if the water cools within a few weeks.', 'İyileşme mümkün, ancak yalnızca su birkaç hafta içinde soğursa.', ['İyileşme mümkün değil, su birkaç hafta içinde soğusa bile.', 'İyileşme mümkün, ancak yalnızca su birkaç ay içinde soğursa.', 'İyileşme mümkün, ancak yalnızca su birkaç hafta içinde ısınırsa.', 'İyileşme kesin, su ne zaman soğursa soğusun.']],
      x: ['Reefs in deeper water, in contrast, showed almost no damage.', 'Buna karşılık daha derin sulardaki resifler neredeyse hiç hasar göstermedi.', ['Buna karşılık daha sığ sulardaki resifler neredeyse hiç hasar göstermedi.', 'Bunun sonucunda daha derin sulardaki resifler neredeyse hiç hasar göstermedi.', 'Buna karşılık daha derin sulardaki resifler ağır hasar gördü.', 'Buna karşılık daha derin sulardaki balıklar neredeyse hiç etkilenmedi.']],
    ...
    { key: 'wages', tt: 'asgari ücret artışının istihdama etkisi',
      i: ['The region raised its minimum wage by twelve per cent in 2018.', 'Bölge 2018\'de asgari ücreti yüzde on iki artırdı.', ['Bölge 2018\'de asgari ücreti yüzde on iki azalttı.', 'Bölge 2018\'de asgari ücreti yüzde yirmi artırdı.', 'Bölge 2008\'de asgari ücreti yüzde on iki artırdı.', 'Merkezi hükümet 2018\'de asgari ücreti yüzde on iki artırdı.']],
      d: ['Economists disagreed about how employment would respond to the increase.', 'İktisatçılar istihdamın bu artışa nasıl tepki vereceği konusunda ayrıştı.', ['İktisatçılar istihdamın bu artışa nasıl tepki vereceği konusunda hemfikirdi.', 'İşçiler istihdamın bu artışa nasıl tepki vereceği konusunda ayrıştı.', 'İktisatçılar ücretlerin bu artışa nasıl tepki vereceği konusunda ayrıştı.', 'İktisatçılar istihdamın geçmiş artışlara nasıl tepki verdiği konusunda ayrıştı.']],
      e: ['Payroll records for the following year show no measurable fall in hiring.', 'Ertesi yılın bordro kayıtları işe alımlarda ölçülebilir bir düşüş göstermiyor.', ['Ertesi yılın bordro kayıtları işe alımlarda ölçülebilir bir düşüş gösteriyor.', 'Ertesi yılın bordro kayıtları ücretlerde ölçülebilir bir düşüş göstermiyor.', 'Önceki yılın bordro kayıtları işe alımlarda ölçülebilir bir düşüş göstermiyor.', 'Ertesi yılın bordro kayıtları işe alımlarda hafif bir artış gösteriyor.']],
      c: ['The finding has since been cited in three separate policy reviews.', 'Bulgu o zamandan beri üç ayrı politika değerlendirmesinde anıldı.', ['Bulgu o zamandan beri üç ayrı politika değerlendirmesinde reddedildi.', 'Bulgu o zamandan beri yalnızca bir politika değerlendirmesinde anıldı.', 'Bulgu o zamandan beri üç ayrı akademik makalede anıldı.', 'Bulgu henüz hiçbir politika değerlendirmesinde anılmadı.']],
      x: ['Small firms in rural districts, however, reported a sharp rise in costs.', 'Ne var ki kırsal ilçelerdeki küçük firmalar maliyetlerde keskin bir artış bildirdi.', ['Ne var ki kırsal ilçelerdeki küçük firmalar maliyetlerde keskin bir düşüş bildirdi.', 'Ne var ki kentsel bölgelerdeki küçük firmalar maliyetlerde keskin bir artış bildirdi.', 'Bu sayede kırsal ilçelerdeki küçük firmalar maliyetlerde keskin bir artış bildirdi.', 'Ne var ki kırsal ilçelerdeki büyük firmalar maliyetlerde keskin bir artış bildirdi.']],
    ...
    { key: 'pollen', tt: 'göl tortullarındaki polen kayıtları',
      i: ['Lake sediments trap pollen grains from the plants growing around them.', 'Göl tortulları çevrede yetişen bitkilerin polen tanelerini hapseder.', ['Göl tortulları çevrede yetişen bitkilerin polen tanelerini serbest bırakır.', 'Nehir tortulları çevrede yetişen bitkilerin polen tanelerini hapseder.', 'Göl tortulları uzak bölgelerdeki bitkilerin polen tanelerini hapseder.', 'Göl suyu çevrede yetişen bitkilerin tohumlarını hapseder.']],
      d: ['Each species leaves a grain with a shape that can be identified under a microscope.', 'Her tür, mikroskop altında tanınabilen bir biçimde tane bırakır.', ['Her tür, çıplak gözle tanınabilen bir biçimde tane bırakır.', 'Bazı türler, mikroskop altında tanınabilen bir biçimde tane bırakır.', 'Her tür, mikroskop altında bile tanınamayan bir biçimde tane bırakır.', 'Her tür, mikroskop altında tanınabilen aynı biçimde tane bırakır.']],
      e: ['Counts from an Anatolian lake show cereal pollen appearing around 8000 BC.', 'Bir Anadolu gölünden alınan sayımlar tahıl polenlerinin MÖ 8000 dolaylarında belirdiğini gösteriyor.', ['Bir Anadolu gölünden alınan sayımlar tahıl polenlerinin MS 8000 dolaylarında belirdiğini gösteriyor.', 'Bir Avrupa gölünden alınan sayımlar tahıl polenlerinin MÖ 8000 dolaylarında belirdiğini gösteriyor.', 'Bir Anadolu gölünden alınan sayımlar tahıl polenlerinin MÖ 3000 dolaylarında belirdiğini gösteriyor.', 'Bir Anadolu gölünden alınan sayımlar tahıl polenlerinin MÖ 8000\'den sonra kaybolduğunu gösteriyor.']],
      c: ['Such records date the spread of farming without relying on any excavation.', 'Bu kayıtlar tarımın yayılışını hiçbir kazıya dayanmadan tarihlendiriyor.', ['Bu kayıtlar tarımın yayılışını yalnızca kazı verileriyle tarihlendiriyor.', 'Bu kayıtlar tarımın kökenini hiçbir kazıya dayanmadan tarihlendiriyor.', 'Bu kazılar tarımın yayılışını hiçbir kayda dayanmadan tarihlendiriyor.', 'Bu kayıtlar tarımın yayılışını tarihlendirmede yetersiz kalıyor.']],
      x: ['Wind can carry pollen for hundreds of kilometres, which complicates the picture.', 'Rüzgâr poleni yüzlerce kilometre taşıyabildiği için tablo karmaşıklaşıyor.', ['Rüzgâr poleni yüzlerce kilometre taşıyabildiği için tablo netleşiyor.', 'Su akıntısı poleni yüzlerce kilometre taşıyabildiği için tablo karmaşıklaşıyor.', 'Rüzgâr poleni birkaç kilometre taşıyabildiği için tablo karmaşıklaşıyor.', 'Rüzgâr poleni yüzlerce kilometre taşıyamadığı için tablo karmaşıklaşıyor.']],
    ...
    { key: 'opensource', tt: 'bakanlığın açık kaynaklı yazılıma geçişi',
      i: ['The ministry replaced its office software with an open-source suite in 2015.', 'Bakanlık 2015\'te ofis yazılımını açık kaynaklı bir paketle değiştirdi.', ['Bakanlık 2015\'te açık kaynaklı yazılımını ticari bir paketle değiştirdi.', 'Bakanlık 2005\'te ofis yazılımını açık kaynaklı bir paketle değiştirdi.', 'Bir belediye 2015\'te ofis yazılımını açık kaynaklı bir paketle değiştirdi.', 'Bakanlık 2015\'te ofis yazılımını açık kaynaklı bir paketle güncelledi, değiştirmedi.']],
      d: ['Licence payments fell immediately, but staff needed retraining.', 'Lisans ödemeleri hemen düştü, ancak personelin yeniden eğitilmesi gerekti.', ['Lisans ödemeleri hemen arttı, ancak personelin yeniden eğitilmesi gerekti.', 'Lisans ödemeleri hemen düştü, ancak personel işten çıkarıldı.', 'Lisans ödemeleri yavaş yavaş düştü, personelin eğitime ihtiyacı olmadı.', 'Lisans ödemeleri hemen düştü ve personelin eğitilmesine gerek kalmadı.']],
      e: ['An internal audit found that support requests doubled during the first six months.', 'Bir iç denetim, destek taleplerinin ilk altı ayda ikiye katlandığını buldu.', ['Bir iç denetim, destek taleplerinin ilk altı ayda yarıya indiğini buldu.', 'Bir dış denetim, destek taleplerinin ilk altı ayda ikiye katlandığını buldu.', 'Bir iç denetim, destek taleplerinin ilk yılda ikiye katlandığını buldu.', 'Bir iç denetim, personel sayısının ilk altı ayda ikiye katlandığını buldu.']],
      c: ['After the second year, both costs and complaints settled below their earlier levels.', 'İkinci yılın ardından hem maliyetler hem şikâyetler eski düzeylerinin altına indi.', ['İkinci yılın ardından hem maliyetler hem şikâyetler eski düzeylerinin üstüne çıktı.', 'İkinci yılın ardından maliyetler indi ama şikâyetler eski düzeyinde kaldı.', 'Birinci yılın ardından hem maliyetler hem şikâyetler eski düzeylerinin altına indi.', 'İkinci yılın ardından hem maliyetler hem personel sayısı eski düzeylerinin altına indi.']],
      x: ['Two departments, nevertheless, kept the old software for legal reasons.', 'Buna karşın iki birim hukuki nedenlerle eski yazılımı kullanmayı sürdürdü.', ['Buna karşın iki birim mali nedenlerle eski yazılımı kullanmayı sürdürdü.', 'Bu sayede iki birim hukuki nedenlerle eski yazılımı kullanmayı sürdürdü.', 'Buna karşın iki birim hukuki nedenlerle yeni yazılımı kullanmayı sürdürdü.', 'Buna karşın on birim hukuki nedenlerle eski yazılımı kullanmayı sürdürdü.']],
    ...
    { key: 'birdsong', tt: 'kent gürültüsünün kuş şarkılarına etkisi',
      i: ['Male songbirds learn their songs from adults during a short period after hatching.', 'Erkek ötücü kuşlar şarkılarını yumurtadan çıktıktan sonraki kısa bir dönemde yetişkinlerden öğrenir.', ['Dişi ötücü kuşlar şarkılarını yumurtadan çıktıktan sonraki kısa bir dönemde yetişkinlerden öğrenir.', 'Erkek ötücü kuşlar şarkılarını yumurtadan çıkmadan önceki kısa bir dönemde yetişkinlerden öğrenir.', 'Erkek ötücü kuşlar şarkılarını doğuştan bilir, öğrenmez.', 'Erkek ötücü kuşlar şarkılarını yumurtadan çıktıktan sonra uzun bir dönem boyunca öğrenir.']],
      d: ['Birds raised in isolation produce a simplified song with the same basic rhythm.', 'Yalıtılmış olarak büyütülen kuşlar aynı temel ritmi taşıyan basitleşmiş bir şarkı üretir.', ['Yalıtılmış olarak büyütülen kuşlar hiç şarkı söylemeyi öğrenemez.', 'Toplu olarak büyütülen kuşlar aynı temel ritmi taşıyan basitleşmiş bir şarkı üretir.', 'Yalıtılmış olarak büyütülen kuşlar farklı bir temel ritimle basitleşmiş bir şarkı üretir.', 'Yalıtılmış olarak büyütülen kuşlar aynı temel ritmi taşıyan karmaşık bir şarkı üretir.']],
      e: ['Recordings from city parks show songs pitched higher than those in nearby forests.', 'Kent parklarından alınan kayıtlar şarkıların yakın ormanlardakinden daha tiz olduğunu gösteriyor.', ['Kent parklarından alınan kayıtlar şarkıların yakın ormanlardakinden daha pes olduğunu gösteriyor.', 'Uzak ormanlardan alınan kayıtlar şarkıların kent parklarındakinden daha tiz olduğunu gösteriyor.', 'Kent parklarından alınan kayıtlar şarkıların yakın ormanlardakiyle aynı perdede olduğunu gösteriyor.', 'Kent parklarından alınan kayıtlar şarkı sürelerinin yakın ormanlardakinden daha uzun olduğunu gösteriyor.']],
      c: ['The difference is best explained by the low-frequency noise of traffic.', 'Bu fark en iyi trafiğin alçak frekanslı gürültüsüyle açıklanıyor.', ['Bu fark en iyi trafiğin yüksek frekanslı gürültüsüyle açıklanıyor.', 'Bu fark en iyi kentteki ışık kirliliğiyle açıklanıyor.', 'Bu fark trafik gürültüsüyle açıklanamıyor.', 'Bu fark en iyi kuşların göç mesafesiyle açıklanıyor.']],
      x: ['Females, on the other hand, show no comparable change in their calls.', 'Öte yandan dişiler çağrılarında benzer bir değişim göstermiyor.', ['Öte yandan dişiler çağrılarında benzer bir değişim gösteriyor.', 'Öte yandan erkekler çağrılarında benzer bir değişim göstermiyor.', 'Bu sayede dişiler çağrılarında benzer bir değişim göstermiyor.', 'Öte yandan dişiler şarkı söylemeyi tamamen bırakıyor.']],
    ...
    { key: 'shipwreck', tt: 'batıktaki yükün gösterdiği ticaret ağı',
      i: ['A merchant ship that sank off the Aegean coast was surveyed in 2019.', 'Ege kıyısı açıklarında batan bir ticaret gemisi 2019\'da incelendi.', ['Karadeniz kıyısı açıklarında batan bir ticaret gemisi 2019\'da incelendi.', 'Ege kıyısı açıklarında batan bir ticaret gemisi 2009\'da incelendi.', 'Ege kıyısı açıklarında batan bir savaş gemisi 2019\'da incelendi.', 'Ege kıyısı açıklarında yol alan bir ticaret gemisi 2019\'da incelendi.']],
      d: ['Its cargo included amphorae from three separate production centres.', 'Yükü arasında üç ayrı üretim merkezinden gelen amforalar bulunuyordu.', ['Yükü arasında tek bir üretim merkezinden gelen amforalar bulunuyordu.', 'Yükü arasında üç ayrı üretim merkezinden gelen heykeller bulunuyordu.', 'Yükü arasında altı ayrı üretim merkezinden gelen amforalar bulunuyordu.', 'Yükünde herhangi bir amfora bulunmuyordu.']],
      e: ['Clay analysis matched two of them to workshops six hundred kilometres apart.', 'Kil analizi bunlardan ikisini altı yüz kilometre uzaklıktaki atölyelerle eşleştirdi.', ['Kil analizi bunlardan ikisini altmış kilometre uzaklıktaki atölyelerle eşleştirdi.', 'Kil analizi bunlardan hepsini altı yüz kilometre uzaklıktaki atölyelerle eşleştirdi.', 'Karbon analizi bunlardan ikisini altı yüz kilometre uzaklıktaki atölyelerle eşleştirdi.', 'Kil analizi bunlardan ikisinin aynı atölyeden geldiğini gösterdi.']],
      c: ['The wreck therefore documents a single voyage across a wide trading network.', 'Böylece batık, geniş bir ticaret ağı boyunca yapılmış tek bir yolculuğu belgeliyor.', ['Böylece batık, geniş bir ticaret ağı boyunca yapılmış birçok yolculuğu belgeliyor.', 'Böylece batık, dar bir bölgesel ticareti belgeliyor.', 'Buna rağmen batık, geniş bir ticaret ağını belgeleyemiyor.', 'Böylece kil analizi, geniş bir ticaret ağı boyunca yapılmış tek bir yolculuğu belgeliyor.']],
      x: ['The hull itself, unfortunately, was too decayed to be dated.', 'Ne yazık ki gövdenin kendisi tarihlendirilemeyecek kadar çürümüştü.', ['Neyse ki gövdenin kendisi kolayca tarihlendirilebildi.', 'Ne yazık ki yükün kendisi tarihlendirilemeyecek kadar çürümüştü.', 'Ne yazık ki gövde tarihlendirilebildi ama menşei belirlenemedi.', 'Ne yazık ki gövdenin kendisi hiç bulunamadı.']],
    ...
    { key: 'literacy', tt: 'yetişkin okuma kurslarının sonuçları',
      i: ['The province opened evening reading courses for adults in 2009.', 'İl, yetişkinler için akşam okuma kurslarını 2009\'da açtı.', ['İl, çocuklar için akşam okuma kurslarını 2009\'da açtı.', 'İl, yetişkinler için sabah okuma kurslarını 2009\'da açtı.', 'İl, yetişkinler için akşam okuma kurslarını 1999\'da açtı.', 'İl, yetişkinler için akşam okuma kurslarını 2009\'da kapattı.']],
      d: ['Attendance was highest among women over forty.', 'Katılım en çok kırk yaş üstü kadınlar arasında yüksekti.', ['Katılım en çok kırk yaş altı kadınlar arasında yüksekti.', 'Katılım en çok kırk yaş üstü erkekler arasında yüksekti.', 'Katılım en çok kırk yaş üstü kadınlar arasında düşüktü.', 'Katılım tüm yaş gruplarında eşit dağılmıştı.']],
      e: ['Follow-up tests recorded a rise of thirty per cent in reading speed after one term.', 'İzleme testleri bir dönem sonunda okuma hızında yüzde otuzluk bir artış kaydetti.', ['İzleme testleri bir dönem sonunda okuma hızında yüzde otuzluk bir düşüş kaydetti.', 'İzleme testleri bir dönem sonunda okuma hızında yüzde onluk bir artış kaydetti.', 'İzleme testleri iki dönem sonunda okuma hızında yüzde otuzluk bir artış kaydetti.', 'İzleme testleri bir dönem sonunda anlama düzeyinde yüzde otuzluk bir artış kaydetti.']],
      c: ['The programme was extended to eleven neighbouring provinces the following year.', 'Program ertesi yıl komşu on bir ile yaygınlaştırıldı.', ['Program ertesi yıl komşu bir ile yaygınlaştırıldı.', 'Program ertesi yıl yalnızca aynı ilde sürdürüldü, yaygınlaştırılmadı.', 'Program beş yıl sonra komşu on bir ile yaygınlaştırıldı.', 'Program ertesi yıl komşu yirmi bir ile yaygınlaştırıldı.']],
      x: ['Enrolment among younger adults, by contrast, remained disappointingly low.', 'Buna karşılık genç yetişkinlerde kayıt oranı hayal kırıklığı yaratacak kadar düşük kaldı.', ['Buna karşılık genç yetişkinlerde kayıt oranı umut verecek kadar yüksek kaldı.', 'Bu sayede genç yetişkinlerde kayıt oranı hayal kırıklığı yaratacak kadar düşük kaldı.', 'Buna karşılık yaşlı yetişkinlerde kayıt oranı hayal kırıklığı yaratacak kadar düşük kaldı.', 'Buna karşılık genç yetişkinlerde kayıt oranı giderek yükseldi.']],
    ...
    { key: 'quake', tt: 'fay hattındaki küçük sarsıntıların izlenmesi',
      i: ['Seismic sensors were installed along the fault in 2012.', 'Fay hattı boyunca sismik sensörler 2012\'de kuruldu.', ['Fay hattı boyunca sismik sensörler 2012\'de söküldü.', 'Fay hattı boyunca hava kalitesi sensörleri 2012\'de kuruldu.', 'Fay hattı boyunca sismik sensörler 2002\'de kuruldu.', 'Şehir merkezinde sismik sensörler 2012\'de kuruldu.']],
      d: ['They record tremors far too small to be felt on the surface.', 'Bu sensörler yüzeyde hissedilemeyecek kadar küçük sarsıntıları kaydediyor.', ['Bu sensörler yalnızca yüzeyde hissedilen büyük sarsıntıları kaydediyor.', 'Bu sensörler yüzeyde hissedilemeyecek kadar küçük sarsıntıları önleyemiyor.', 'Bu sensörler derinlerde hissedilemeyecek kadar küçük sarsıntıları kaydediyor.', 'Bu sensörler yüzeyde zar zor hissedilen sarsıntıları kaydediyor.']],
      e: ['The records reveal that small tremors cluster in the weeks before larger events.', 'Kayıtlar, küçük sarsıntıların büyük olaylardan önceki haftalarda kümelendiğini ortaya koyuyor.', ['Kayıtlar, küçük sarsıntıların büyük olaylardan sonraki haftalarda kümelendiğini ortaya koyuyor.', 'Kayıtlar, büyük sarsıntıların küçük olaylardan önceki haftalarda kümelendiğini ortaya koyuyor.', 'Kayıtlar, küçük sarsıntıların rastgele dağıldığını, kümelenmediğini ortaya koyuyor.', 'Kayıtlar, küçük sarsıntıların büyük olaylardan önceki aylarda kümelendiğini ortaya koyuyor.']],
      c: ['This pattern has not yet produced a reliable method of prediction.', 'Bu örüntü henüz güvenilir bir tahmin yöntemi doğurmadı.', ['Bu örüntü artık güvenilir bir tahmin yöntemi doğurdu.', 'Bu örüntü henüz güvenilir bir önleme yöntemi doğurmadı.', 'Bu örüntüler henüz hiçbir bilim insanının dikkatini çekmedi.', 'Bu örüntü, tahmin yöntemini güvenilmez kılmıştır.']],
      x: ['Similar clusters, however, appear in years when no large earthquake follows.', 'Ne var ki benzer kümelenmeler büyük bir depremin izlemediği yıllarda da görülüyor.', ['Bu sayede benzer kümelenmeler büyük bir depremin izlemediği yıllarda da görülüyor.', 'Ne var ki benzer kümelenmeler yalnızca büyük bir depremin izlediği yıllarda görülüyor.', 'Ne var ki farklı kümelenmeler büyük bir depremin izlemediği yıllarda da görülüyor.', 'Ne var ki benzer kümelenmeler artık hiç görülmüyor.']],
    ...
    { key: 'museumlight', tt: 'müze aydınlatmasının eserlere etkisi',
      i: ['The gallery replaced its halogen lamps with low-energy diodes in 2017.', 'Galeri 2017\'de halojen lambalarını düşük enerjili diyotlarla değiştirdi.', ['Galeri 2017\'de düşük enerjili diyotlarını halojen lambalarla değiştirdi.', 'Galeri 2007\'de halojen lambalarını düşük enerjili diyotlarla değiştirdi.', 'Müze 2017\'de halojen lambalarını yüksek enerjili diyotlarla değiştirdi.', 'Kütüphane 2017\'de halojen lambalarını düşük enerjili diyotlarla değiştirdi.']],
      d: ['The new lamps emit almost no ultraviolet light.', 'Yeni lambalar neredeyse hiç morötesi ışık yaymıyor.', ['Yeni lambalar yoğun biçimde morötesi ışık yayıyor.', 'Eski lambalar neredeyse hiç morötesi ışık yaymıyor.', 'Yeni lambalar neredeyse hiç kızılötesi ışık yaymıyor.', 'Yeni lambalar morötesi ışığı süzerek azaltıyor ama tamamen kesmiyor.']],
      e: ['Conservators measured a marked slowdown in the fading of the textiles.', 'Koruma uzmanları tekstillerdeki solmada belirgin bir yavaşlama ölçtü.', ['Koruma uzmanları tekstillerdeki solmada belirgin bir hızlanma ölçtü.', 'Ziyaretçiler tekstillerdeki solmada belirgin bir yavaşlama fark etti.', 'Koruma uzmanları tabloların solmasında belirgin bir yavaşlama ölçtü.', 'Koruma uzmanları tekstillerin solmasının tamamen durduğunu ölçtü.']],
      c: ['Other museums in the country have since adopted the same lighting.', 'Ülkedeki diğer müzeler o tarihten beri aynı aydınlatmayı benimsedi.', ['Ülkedeki diğer müzeler o tarihten beri bu aydınlatmayı reddetti.', 'Yurt dışındaki müzeler o tarihten beri aynı aydınlatmayı benimsedi.', 'Ülkedeki diğer müzeler o tarihten beri farklı bir aydınlatma benimsedi.', 'Bu galeri o tarihten beri diğer müzelerin aydınlatmasını benimsedi.']],
      x: ['Visitors, on the other hand, complained that the colours looked flat.', 'Öte yandan ziyaretçiler renklerin sönük göründüğünden yakındı.', ['Öte yandan ziyaretçiler renklerin canlı göründüğünden memnundu.', 'Öte yandan küratörler renklerin sönük göründüğünden yakındı.', 'Bu sayede ziyaretçiler renklerin sönük göründüğünden yakındı.', 'Öte yandan ziyaretçiler ışıkların çok parlak olduğundan yakındı.']],
    ...
    { key: 'bicycle', tt: 'kampüsteki bisiklet ödünç verme programı',
      i: ['The university opened a bicycle lending scheme for its students in 2014.', 'Üniversite 2014\'te öğrencileri için bisiklet ödünç verme programı başlattı.', ['Üniversite 2014\'te öğrencileri için bisiklet satış programı başlattı.', 'Üniversite 2004\'te öğrencileri için bisiklet ödünç verme programı başlattı.', 'Belediye 2014\'te öğrenciler için bisiklet ödünç verme programı başlattı.', 'Üniversite 2014\'te personeli için bisiklet ödünç verme programı başlattı.']],
      d: ['Bicycles could be borrowed from any of four stations on the campus.', 'Bisikletler kampüsteki dört istasyonun herhangi birinden ödünç alınabiliyordu.', ['Bisikletler kampüsteki tek bir istasyondan ödünç alınabiliyordu.', 'Bisikletler kampüs dışındaki dört istasyonun herhangi birinden ödünç alınabiliyordu.', 'Bisikletler kampüsteki sekiz istasyonun herhangi birinden ödünç alınabiliyordu.', 'Bisikletler yalnızca kütüphane önündeki istasyondan ödünç alınabiliyordu.']],
      e: ['Usage data showed that ninety per cent of trips took less than ten minutes.', 'Kullanım verileri yolculukların yüzde doksanının on dakikadan kısa sürdüğünü gösterdi.', ['Kullanım verileri yolculukların yüzde doksanının on dakikadan uzun sürdüğünü gösterdi.', 'Kullanım verileri yolculukların yüzde onunun on dakikadan kısa sürdüğünü gösterdi.', 'Kullanım verileri yolculukların yüzde doksanının otuz dakikadan kısa sürdüğünü gösterdi.', 'Kullanım verileri istasyon sayısının yüzde doksan arttığını gösterdi.']],
      c: ['The scheme was therefore redesigned around short trips between faculties.', 'Bu yüzden program fakülteler arası kısa yolculuklara göre yeniden tasarlandı.', ['Bu yüzden program şehir merkezine uzun yolculuklara göre yeniden tasarlandı.', 'Buna rağmen program yeniden tasarlanmadı.', 'Bu yüzden program tamamen iptal edildi.', 'Bu yüzden istasyonlar fakülteler arası kısa yolculuklara göre yeniden tasarlandı.']],
      x: ['Trips to the city centre, however, almost never used the scheme.', 'Ne var ki kent merkezine yapılan yolculuklarda program neredeyse hiç kullanılmadı.', ['Ne var ki kent merkezine yapılan yolculuklarda program sıklıkla kullanıldı.', 'Bu sayede kent merkezine yapılan yolculuklarda program neredeyse hiç kullanılmadı.', 'Ne var ki fakülteler arası yolculuklarda program neredeyse hiç kullanılmadı.', 'Ne var ki kent merkezine yapılan yolculuklar tamamen yasaklandı.']],
    ...
    { key: 'terrace', tt: 'taş terasların toprak kaybını önlemesi',
      i: ['Farmers on the steep valley slopes have built stone terraces for centuries.', 'Dik vadi yamaçlarındaki çiftçiler yüzyıllardır taş teraslar inşa ediyor.', ['Düz vadi tabanındaki çiftçiler yüzyıllardır taş teraslar inşa ediyor.', 'Dik vadi yamaçlarındaki çiftçiler yalnızca son yıllarda taş teraslar inşa ediyor.', 'Dik vadi yamaçlarındaki çobanlar yüzyıllardır taş teraslar inşa ediyor.', 'Dik vadi yamaçlarındaki çiftçiler yüzyıllardır ahşap teraslar inşa ediyor.']],
      d: ['The terraces hold rainwater long enough for it to sink into the soil.', 'Teraslar yağmur suyunu toprağa sızacak kadar uzun süre tutuyor.', ['Teraslar yağmur suyunu hemen akıtacak kadar kısa süre tutuyor.', 'Teraslar yağmur suyunu buharlaşacak kadar uzun süre tutuyor.', 'Duvarlar yağmur suyunu toprağa sızacak kadar uzun süre tutuyor.', 'Teraslar kar suyunu toprağa sızacak kadar uzun süre tutuyor.']],
      e: ['Fields behind intact terraces lost half as much topsoil in the 2020 floods.', '2020 sellerinde sağlam terasların gerisindeki tarlalar yarı yarıya az üst toprak yitirdi.', ['2020 sellerinde sağlam terasların gerisindeki tarlalar iki kat fazla üst toprak yitirdi.', '2010 sellerinde sağlam terasların gerisindeki tarlalar yarı yarıya az üst toprak yitirdi.', '2020 sellerinde yıkık terasların gerisindeki tarlalar yarı yarıya az üst toprak yitirdi.', '2020 sellerinde sağlam terasların önündeki tarlalar yarı yarıya az üst toprak yitirdi.']],
      c: ['Restoring the abandoned walls is now part of the regional flood plan.', 'Terk edilmiş duvarların onarımı artık bölgesel taşkın planının bir parçası.', ['Terk edilmiş duvarların yıkımı artık bölgesel taşkın planının bir parçası.', 'Yeni duvarların inşası artık bölgesel taşkın planının bir parçası.', 'Terk edilmiş duvarların onarımı artık ulusal taşkın planının bir parçası.', 'Terk edilmiş duvarların onarımı bölgesel taşkın planından çıkarıldı.']],
      x: ['Maintaining the walls by hand, however, is slow and expensive work.', 'Ne var ki duvarları elle bakımda tutmak yavaş ve pahalı bir iş.', ['Ne var ki duvarları makineyle bakımda tutmak yavaş ve pahalı bir iş.', 'Bu sayede duvarları elle bakımda tutmak yavaş ve pahalı bir iş.', 'Ne var ki duvarları elle bakımda tutmak hızlı ve ucuz bir iş.', 'Ne var ki duvarları elle bakımda tutmaya artık gerek kalmadı.']],
  ...
  /* ── Çeviri ─────────────────────────────────────────────────────────────── */
  // Çeldiriciler kaynak cümlenin kendisinin bozulmuş çevirileridir (özne/nesne
  // yer değişimi, olumsuzluk, zaman/sayı kayması, ters bağlaç): konu aynı
  // kalır ama tek bir sözcük ya da ilişki değiştiği için gerçek çeviri
  // bilgisi gerekir. Başka bir cümleyi tanımakla doğru şıkka ulaşılamaz.
  function buildTranslation(need) {
    ...
    for (let k = 0; out.length < need; k++) {
      ...
      const wrong = t[slot][2];
      out.push(mc(`xp_tr_${k + 1}`,
        `Aşağıdaki cümlenin Türkçeye en uygun çevirisini bulunuz:<br><br><strong>"${t[slot][EN]}"</strong>`,
        correct, wrong, k,
        Object.assign({
          explanation: `Cümlenin öznesi ve yüklemi birebir korunmalıdır: "${t[slot][EN]}" ifadesinin karşılığı "${correct}" seçeneğidir. Diğer seçenekler aynı cümlenin bir ayrıntısını (özne, sayı, zaman veya olumsuzluk) değiştirerek anlamı kaydırır.`
        }, tag('translation'))));
    }
    return out;
  }
