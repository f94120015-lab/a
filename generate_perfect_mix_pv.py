...
# Preposition extractor for Phrasal Verbs
def get_prep_info(verb_str):
    # e.g., "keep on" -> base "keep", prep "on"
    # "cut down (on)" -> base "cut down", prep "on"
    # "make up for" -> base "make up", prep "for"
    parts = verb_str.replace('(', '').replace(')', '').split()
    if len(parts) == 2:
...
code_buffer = []
code_buffer.append("  // ============================================================")
code_buffer.append("  // BÖLÜM 60 / UNIT 57: 148 PHRASAL VERBS PERFECT MIX MASTERCLASS")
code_buffer.append("  // ============================================================")
...
for l_idx, chunk in enumerate(chunks):
    ...
    for item_idx, item in enumerate(chunk):
        ...
        # 3 Rotational High Quality Question Types (No flawed Error-Spotting!):
        # 0: fill-blank-dropdown (Açılır Kutudan Seçim)
        # 1: multiple-choice (Çoktan Seçmeli Bağlam Testi)
        # 2: preposition-magnet / Synonym & Preposition Focus (Edat Mıknatısı & Akademik Eş Anlamlı)
        
        q_type_selector = item_idx % 3
        
        if q_type_selector == 0:
            # 1. fill-blank-dropdown (Açılır Kutudan Seçim)
            q_obj = {
                "id": f"u60_l{lesson_num}_q{item_idx+1}_fb",
                "type": "fill-blank-dropdown",
                "prompt": f"[Beşeri Bilimler - Soru {item['no']}/148 - Boşluk Doldurma] Cümledeki boşluğa en uygun deyimsel fiili seçin:",
                "sentence": sent,
            ...
            # 2. multiple-choice (Cümle İçi Akademik Bağlam Testi)
            ...
            # 3. preposition-magnet / Synonym Focus (Edat Kilitleri & Akademik Eş Anlamlısı)
            ...
            opts_prep = [
                f"<span style='color: #ff6b6b; font-weight: bold;'>{correct_prep}</span> ➔ {v} ({m} / Eş Anlam: {syn})",
            ...
            q_obj = {
                ...
                "type": "multiple-choice",
                "prompt": f"[Beşeri Bilimler - Soru {item['no']}/148 - Edat & Akademik Eş Anlam Kilidi] '{base_v}' fiilini takip eden en uygun edatı (Preposition) ve akademik eş anlamlısını seçin:<br><br><strong>'{prep_sent}'</strong>",
                "options": opts_prep,
                "correctIndex": 0,
                "explanation": f"'{base_v}' fiili bu cümlede '{m}' (eş anlamı: {syn}) anlamını vermek için <span style='color: #ff6b6b; font-weight: bold;'>{correct_prep}</span> edatı ile birleşerek <span style='color: #ff6b6b; font-weight: bold;'>{v}</span> deyimsel fiilini oluşturur."
            }
...
print("Perfect Mix PV Code generated successfully!")
