...
# 1. Cümleler (Sorted by length later)
img_data = [
...
total_sentences = img_data + add_data

# Sort by length
total_sorted = sorted(total_sentences, key=lambda x: len(x['en']))

# Verify count is exactly 120
print(f"Total sorted sentences: {len(total_sorted)}")

# Partition into 3 lessons of 40 sentences each
lesson1 = total_sorted[0:40]
lesson2 = total_sorted[40:80]
lesson3 = total_sorted[80:120]

def format_sentences_js(name, data_list):
    lines = [f"const {name} = ["]
...
lesson1_js = format_sentences_js("unit18Lesson1SentencesRaw", lesson1)
lesson2_js = format_sentences_js("unit18Lesson2SentencesRaw", lesson2)
lesson3_js = format_sentences_js("unit18Lesson3SentencesRaw", lesson3)

# 2. Modify data.js
print("Modifying data.js...")
with open("data.js", "r", encoding="utf-8") as f:
    data_content = f.read()

# Replace unit18 raw arrays
...
start_marker = "const unit18LessonSentences = {"
end_marker = "const unit19LessonSentences = {"
start_idx = data_content.find(start_marker)
end_idx = data_content.find(end_marker)

if start_idx != -1 and end_idx != -1:
    data_content = data_content[:start_idx] + raw_sentences_js + "\n\n" + data_content[end_idx:]
    print("Replaced unit18LessonSentences in memory.")
else:
...
new_map = """  18: {
    1: { exercises: [
      buildCustom10QuestionExercises(unit18Lesson1SentencesRaw, 18, 43, 1, 0),
      buildCustom10QuestionExercises(unit18Lesson1SentencesRaw, 18, 43, 2, 10),
      buildCustom10QuestionExercises(unit18Lesson1SentencesRaw, 18, 43, 3, 20),
      buildCustom10QuestionExercises(unit18Lesson1SentencesRaw, 18, 43, 4, 30)
    ] },
    2: { exercises: [
      buildCustom10QuestionExercises(unit18Lesson2SentencesRaw, 18, 44, 1, 0),
      buildCustom10QuestionExercises(unit18Lesson2SentencesRaw, 18, 44, 2, 10),
      buildCustom10QuestionExercises(unit18Lesson2SentencesRaw, 18, 44, 3, 20),
      buildCustom10QuestionExercises(unit18Lesson2SentencesRaw, 18, 44, 4, 30)
    ] },
    3: { exercises: [
      buildCustom10QuestionExercises(unit18Lesson3SentencesRaw, 18, 45, 1, 0),
      buildCustom10QuestionExercises(unit18Lesson3SentencesRaw, 18, 45, 2, 10),
      buildCustom10QuestionExercises(unit18Lesson3SentencesRaw, 18, 45, 3, 20),
      buildCustom10QuestionExercises(unit18Lesson3SentencesRaw, 18, 45, 4, 30)
    ] }
  },"""

alt_start = data_content.find("  18: {")
if alt_start != -1:
    alt_end = data_content.find("  },", alt_start)
    if alt_end != -1:
        alt_end += len("  },")
        data_content = data_content[:alt_start] + new_map + data_content[alt_end:]
        print("Replaced map for unit 18.")
    else:
        print("Error: Could not find closing bracket for unit 18 map.")
        exit(1)
else:
    print("Error: Could not find map section for unit 18.")
    exit(1)

# Update rawTopics description for Unit 18
new_topic = """  {
...
alt_idx = data_content.find('title: "XVIII. Soru Kelimesinden Sonra Gelen Mastar (Sayfa 124)"')
if alt_idx != -1:
    block_end = data_content.find("  },", alt_idx)
    if block_end != -1:
        block_end += len("  },")
        block_start = data_content.rfind("  {", 0, alt_idx)
        if block_start != -1:
            data_content = data_content[:block_start] + new_topic + data_content[block_end:]
            print("Replaced rawTopics for Unit 18.")
        else:
            print("Error: Could not find block start for Unit 18 topic.")
            exit(1)
    else:
        print("Error: Could not find block end for Unit 18 topic.")
        exit(1)
else:
    print("Error: Could not find rawTopic entry for Unit 18.")
    exit(1)
...
# 3. Modify app.js dictionary and unlocks
...
new_dict_body = "\n".join(new_body_lines)
app_js = app_js[:match.start()] + dict_start + new_dict_body + dict_end + app_js[match.end():]
print("app.js dictionary updated.")

# Unlock Unit 18 in Sets
app_js = app_js.replace("const notUploadedUnits = new Set([18, 19, 20, 21, 22, 23, 24, 25]);", "const notUploadedUnits = new Set([19, 20, 21, 22, 23, 24, 25]);")
app_js = app_js.replace("const notUploadedUnitsPopover = new Set([18, 19, 20, 21, 22, 23, 24, 25]);", "const notUploadedUnitsPopover = new Set([19, 20, 21, 22, 23, 24, 25]);")
print("Unit 18 unlocked in Set definitions.")
