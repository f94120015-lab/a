...
const rawData = {
  ...
  "3": {
    ...
    academic: [
      ...
      { en: "In many cases, university professors publish qualitative monographs to establish uniform standards for advanced grammar pedagogy frameworks.", tr: "Çoğu durumda, university professors, ileri düzey dil bilgisi pedagojisi çerçeveleri için tek tip standartlar oluşturmak amacıyla nitel monografiler yayınlarlar.", word: "In many cases", options: ["In many cases", "In some cases", "Even if", "Otherwise"] },
...
function makeMCQuestion(qId, s, allSentences) {
  // Pull translations from other sentences in the same lesson
  const otherTranslations = allSentences
    .filter(item => item.tr !== s.tr)
    .map(item => item.tr);

  // Pick 3 unique translations from other sentences
...
function makeWordBankQuestion(qId, s) {
  const blanked = blankWord(s.en, s.word);
  ...
  const fallbacks = ["otherwise", "although", "such as", "for example"];
  fallbacks.forEach(f => {
  ...
  const finalOpts = shuffle(uniqueOpts.slice(0, 3));
...
function generateLessonExercises(lessonIdStr, rawLesson) {
  ...
  const allRaw = [...rawLesson.micro, ...rawLesson.academic];
  allRaw.forEach(s => {
    const blanked = blankWord(s.en, s.word).trim();
    const isEntirelyBlank = blanked === "___" || blanked === "___." || blanked === "___,";
    if (isEntirelyBlank) {
      phrasesPool.push({ ...s, isEntirelyBlank: true });
    } else {
      sentencesPool.push({ ...s, isEntirelyBlank: false });
    }
  });

  phrasesPool.sort((a, b) => a.en.length - b.en.length);
  sentencesPool.sort((a, b) => a.en.length - b.en.length);

  // Divide sentencesPool into 3 progressive subsets to ensure 100% uniqueness
  // Ex 1 Pool: shortest sentences
  // Ex 2 Pool: medium sentences
  // Ex 3 Pool: longest sentences
  const sliceSize = Math.floor(sentencesPool.length / 3);
  const ex1Pool = sentencesPool.slice(0, sliceSize);
  const ex2Pool = sentencesPool.slice(sliceSize, sliceSize * 2);
  const ex3Pool = sentencesPool.slice(sliceSize * 2);
  ...
  // EXERCISE 1
  {
    ...
    // 2 Matching questions (8 pairs total)
    const matchCount = Math.min(8, phrasesPool.length);
    for (let i = 0; i < matchCount; i++) {
      matchingPairs.push({
        left: phrasesPool[i].tr,
        right: phrasesPool[i].en
      });
    }
    // If not enough phrases, grab from ex1Pool
    if (matchingPairs.length < 8) {
      const extraNeeded = 8 - matchingPairs.length;
      for (let i = 0; i < Math.min(extraNeeded, ex1Pool.length); i++) {
        matchingPairs.push({
          left: ex1Pool[i].tr,
          right: ex1Pool[i].en
        });
    ...
    // Cloze & Word Bank from ex1Pool
    const ex1Sentences = shuffle(ex1Pool);
    let clozeIdx = 0;
    
    // 2 MC translation (without commas)
    const mcCandidates = ex1Sentences.filter(s => !s.en.includes(","));
    const mcUsed = [];
    mcCandidates.slice(0, 2).forEach((s, idx) => {
      questions.push(makeMCQuestion(`u${unitId}l${lessonId}_ex1_mc_${idx}`, s, allRaw));
      mcUsed.push(s);
    });

    // 4 Cloze
    const clozeCandidates = ex1Sentences.filter(s => !mcUsed.includes(s));
    clozeCandidates.slice(0, 4).forEach((s, idx) => {
      questions.push(makeClozeQuestion(`u${unitId}l${lessonId}_ex1_cloze_${idx}`, s, idx % 2 === 0));
    });

    // 2 Word Bank
    const wbCandidates = clozeCandidates.slice(4);
    wbCandidates.slice(0, 2).forEach((s, idx) => {
      questions.push(makeWordBankQuestion(`u${unitId}l${lessonId}_ex1_wb_${idx}`, s));
    });

    // Replenish from sentencesPool fallback if needed
    if (questions.length < 10) {
      const currentIds = new Set(questions.map(q => q.id));
      const pool = shuffle(sentencesPool);
      for (let i = 0; i < pool.length; i++) {
        if (questions.length >= 10) break;
        const s = pool[i];
        const qId = `u${unitId}l${lessonId}_ex1_rep_${i}`;
        const qObj = makeClozeQuestion(qId, s, i % 2 === 0);
        if (!currentIds.has(qObj.id)) questions.push(qObj);
      }
    }
    ...
    exercises.push({
      ...
      description: `Temel düzey cümleler ve yapılarla pratik.`,
  ...
  {
    const questions = [];
    const ex2Sentences = shuffle(ex2Pool);

    // 2 MC translation (without commas)
    const mcCandidates = ex2Sentences.filter(s => !s.en.includes(","));
    const mcUsed = [];
    mcCandidates.slice(0, 2).forEach((s, idx) => {
    ...
    // 4 Cloze
    const clozeCandidates = ex2Sentences.filter(s => !mcUsed.includes(s));
    clozeCandidates.slice(0, 4).forEach((s, idx) => {
    ...
    // 4 Word Bank
    const wbCandidates = clozeCandidates.slice(4);
    wbCandidates.slice(0, 4).forEach((s, idx) => {
      questions.push(makeWordBankQuestion(`u${unitId}l${lessonId}_ex2_wb_${idx}`, s));
    });
    ...
    if (questions.length < 10) {
      ...
      for (let i = 0; i < pool.length; i++) {
        if (questions.length >= 10) break;
        const s = pool[i];
        const qId = `u${unitId}l${lessonId}_ex2_rep_${i}`;
        const qObj = makeClozeQuestion(qId, s, i % 2 === 0);
        if (!currentIds.has(qObj.id)) questions.push(qObj);
      }
    }
    ...
    exercises.push({
      ...
      title: `Alıştırma 2: Orta Düzey Akademik Yapılar`,
      description: `Orta düzey akademik yapılarla pratik.`,
      questions: questions.slice(0, 10)
    });
  }
  ...
  {
    const questions = [];
    const ex3Sentences = shuffle(ex3Pool);

    // Sort by length to get the 2 longest for Translation Text at the end
    ex3Sentences.sort((a, b) => b.en.length - a.en.length);
    const transCandidates = ex3Sentences.slice(0, 2);
    const regularCandidates = ex3Sentences.slice(2);

    // 2 MC translation (without commas)
    const mcCandidates = regularCandidates.filter(s => !s.en.includes(","));
    const mcUsed = [];
    mcCandidates.slice(0, 2).forEach((s, idx) => {
    ...
    // 4 Cloze
    const clozeCandidates = regularCandidates.filter(s => !mcUsed.includes(s));
    clozeCandidates.slice(0, 4).forEach((s, idx) => {
    ...
    const wbCandidates = clozeCandidates.slice(4);
    wbCandidates.slice(0, 2).forEach((s, idx) => {
      questions.push(makeWordBankQuestion(`u${unitId}l${lessonId}_ex3_wb_${idx}`, s));
    });

    // 2 Free Translation Texts at the end
    transCandidates.forEach((s, idx) => {
      questions.push({
        id: `u${unitId}l${lessonId}_ex3_tx_et_${idx}`,
    ...
    if (questions.length < 10) {
      ...
      for (let i = 0; i < pool.length; i++) {
        if (questions.length >= 10) break;
        const s = pool[i];
        const qId = `u${unitId}l${lessonId}_ex3_rep_${i}`;
        const qObj = makeClozeQuestion(qId, s, i % 2 === 0);
        if (!currentIds.has(qObj.id)) {
          // Insert before the translation-text questions (which are at the end)
          const textIdx = questions.findIndex(q => q.type === "translation-text");
          if (textIdx !== -1) {
            questions.splice(textIdx, 0, qObj);
          } else {
            questions.push(qObj);
          }
        }
    ...
    exercises.push({
      ...
      title: `Alıştırma 3: İleri Düzey Akademik Analiz`,
      description: `İleri düzey akademik yapılarla pratik.`,
      questions: questions.slice(0, 10)
    });
  }
...
console.log("data.js updated successfully with Unit 31 exercises (Version 2)!");
