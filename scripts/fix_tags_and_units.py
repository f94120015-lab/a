...
# Define Unit 6 (Bölüm 1: Temel Yapılar)
unit6_data = {
  "1": {
    "exercises": [
      {
...
# Define Unit 1 (Bölüm 2: İsim ve Edat Yapıları)
unit1_data = {
  "1": {
    "exercises": [
      {
...
# Define Unit 2 (Bölüm 3: Fiil ve Edat Yapıları)
unit2_data = {
  "1": {
    "exercises": [
      {
...
# 1. Update unitSentencesMap["6"], ["1"], ["2"]
# Find const unitSentencesMap = {
map_start = content.find("const unitSentencesMap = {")
if map_start != -1:
    # Now replace "6", "1", "2" entries specifically inside unitSentencesMap or rebuild them
    print("Found unitSentencesMap!")
    
    # We will serialize unit6_data, unit1_data, unit2_data to JSON strings
    u6_json = json.dumps(unit6_data, ensure_ascii=False, indent=2)
    u1_json = json.dumps(unit1_data, ensure_ascii=False, indent=2)
    u2_json = json.dumps(unit2_data, ensure_ascii=False, indent=2)
    
    # Replace unitSentencesMap["6"], unitSentencesMap["1"], unitSentencesMap["2"] wherever they appear as keys in map
    # Or clean up unitSentencesMap completely to have clean "6", "1", "2"
    
    # Let's check regex replacement for "6": {...}, "1": {...}, "2": {...}
    # First, let's fix any occurrences of "Fiil ve Edat Takımları" or "Fiil + Edat Yapısı" in Unit 1 questions
    content_fixed = content
    
    # Replace unitSentencesMap["1"] block:
    pattern_u1 = r'("1"\s*:\s*\{).*?(\n\s*\},?\n\s*"(2|6|7|8|9|10|70)"\s*:\s*\{)'
    # Also clean up duplicate or misplaced "1": { "1": { ...
    
    # Let's replace unitSentencesMap contents after const unitSentencesMap = {
    # Let's find end of const unitSentencesMap = { ... };
    map_body_start = map_start + len("const unitSentencesMap = {")
    
    # Let's update unitSentencesMap["6"], unitSentencesMap["1"], unitSentencesMap["2"]
    # We can do targeted string updates:
    
    # 1. Replace "6": { ... } in unitSentencesMap
    # 2. Replace "1": { ... } in unitSentencesMap
    # 3. Replace "2": { ... } in unitSentencesMap
    
    # Let's do a clean regex replacement for unitSentencesMap["6"], ["1"], ["2"]
    # To be 100% safe, let's update "6": u6_json, "1": u1_json, "2": u2_json
    
    # Let's replace "1": { ... } at line 34846 and line 31237
    # Notice line 31237 had `"70": { "1": { "1": {` -> let's clean up line 31237 too!
    
    # Let's construct a python approach to replace unitSentencesMap keys cleanly
    # Let's find "1": { ... } in unitSentencesMap
    
    # Simple regex for "1": { ... }
    content_fixed = re.sub(r'"1"\s*:\s*\{\s*"1"\s*:\s*\{', '"1": {', content_fixed)
    
    # Update unitSentencesMap["6"]
    content_fixed = re.sub(r'"6"\s*:\s*\{[^}]*?"exercises".*?\}\s*\}\s*\}\s*\}', '"6": ' + u6_json, content_fixed, flags=re.DOTALL)
    
    # Update unitSentencesMap["1"]
    content_fixed = re.sub(r'"1"\s*:\s*\{[^}]*?"exercises".*?\}\s*\}\s*\}\s*\}', '"1": ' + u1_json, content_fixed, flags=re.DOTALL)
    
    # Update unitSentencesMap["2"]
    content_fixed = re.sub(r'"2"\s*:\s*\{[^}]*?"exercises".*?\}\s*\}\s*\}\s*\}', '"2": ' + u2_json, content_fixed, flags=re.DOTALL)

    # Global cleanup of any leftover "Fiil ve Edat Takımları" in Unit 1 questions
    # If a question has "u1l" in id and contains "Fiil ve Edat Takımları", replace with "İsim ve Edat Yapıları"
    def fix_q_tags(m):
        block = m.group(0)
        if '"id": "u1' in block or '"u1l' in block:
            block = block.replace('"Fiil ve Edat Takımları"', '"İsim ve Edat Yapıları"')
            block = block.replace('"Fiil + Edat Yapısı"', '"İsim + Edat Yapısı"')
        return block

    content_fixed = re.sub(r'\{\s*"id"\s*:\s*"u1[^}]+\}', fix_q_tags, content_fixed)

    with open(file_path, "w", encoding="utf-8") as f:
        f.write(content_fixed)
    print("Successfully fixed tags and updated data.js!")
else:
    print("unitSentencesMap not found!")
